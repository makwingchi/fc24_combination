...
CLUB = [["FC Bayern München"], ["Ajax"]]
NUM_CLUB = [1, 1]  # Total players from i^th list >= NUM_CLUB[i]

MAX_NUM_CLUB = 3  # Same Club Count: Max X / Max X Players from the Same Club
MIN_NUM_CLUB = 4  # Same Club Count: Min X / Min X Players from the Same Club
NUM_UNIQUE_CLUB = [4, "Min"]  # Clubs: Max / Min / Exactly X

LEAGUE = [["Ligue 1 Uber Eats"]]
NUM_LEAGUE = [1]  # Total players from i^th list >= NUM_LEAGUE[i]
...
NUM_RARITY_2 = [3]   # Total players from i^th Rarity >= NUM_RARITY_2[i]

SQUAD_RATING = 77 # Squad Rating: Min XX

MIN_OVERALL = [83]
NUM_MIN_OVERALL = [1]  # Minimum OVR of XX : Min X

CHEMISTRY = 10  # Squad Total Chemistry Points: Min X
